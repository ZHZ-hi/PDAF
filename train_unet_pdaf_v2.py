"""
PDAF Training Script v2 — fixes for:
1. normalize_batch: use fixed ImageNet statistics (no batch-wise dependency)
2. prior_constraint_loss: allow gradient flow to LPE via z_tilde (removed detach)
3. DPE dual-mode training: both z_tilde denoising AND pure Gaussian prior mode
"""
...
# ---------------------------------------------------------------------------
# Normalization — FIX #1: per-image spatial mean/std (matching baseline U-Net).
#
# Original code used batch-wise mean/std (dim=(2,3) over the whole batch),
# which is noisy when batch_size=4.
# Fix: normalize each image independently using its own spatial statistics,
# exactly as train_basic_unet.py does.
# ---------------------------------------------------------------------------


def normalize_image_tensor(images: torch.Tensor) -> torch.Tensor:
    """Per-image spatial mean/std normalization. Input: (B, C, H, W)."""
    # axis=(2,3) = spatial H,W dims → per-image, per-channel stats → (B, C, 1, 1)
...
def kl_divergence_loss(mu: torch.Tensor, logvar: torch.Tensor) -> torch.Tensor:
    ...
    Clamp logvar for numerical stability to prevent exp(logvar) explosion.
    """
...
def semantic_consistency_loss(
    ...
    logits_tilde is trained only via task_loss, not sc_loss.
    Supervising both branches with sc_loss was redundant and may have caused
    gradient conflict between the two branches.

    The teacher source prediction serves as the structural reference for the
    pseudo-target domain — we expect the inference branch to match it.
    """
...
def prior_constraint_loss(z_hat: torch.Tensor, z_tilde: torch.Tensor, lpe_grad: bool = True) -> torch.Tensor:
    """Prior constraint term.

    FIX #2: when lpe_grad=True, gradient flows to BOTH DPE and LPE.
    When lpe_grad=False (default original behavior), only DPE receives gradient.
    """
    target = z_tilde if lpe_grad else z_tilde.detach()
    return F.smooth_l1_loss(torch.tanh(z_hat), torch.tanh(target))


def prior_constraint_loss_lpe_only(z_hat: torch.Tensor, z_tilde: torch.Tensor) -> torch.Tensor:
    """Alternative: only LPE receives gradient (z_hat.detach()), DPE does not.
    Use this if you want to isolate DPE learning from the prior constraint.
    """
    return F.smooth_l1_loss(torch.tanh(z_hat), torch.tanh(z_tilde.detach()))
...
def compute_metrics(prob: np.ndarray, target: np.ndarray, fov: np.ndarray) -> dict[str, float]:
    ...
    # Soft dice: differentiable version of Dice using probabilities
...
def train_pdaf(
    ...
    # FIX #3: DPE dual-mode training
    # When dpe_dual_mode=True, DPE receives BOTH:
    #   (a) z_tilde denoising (standard DDPM training from paired data)
    #   (b) pure Gaussian denoising (inference-like mode, no paired target)
    ...
    for epoch in range(1, stage_cfg["epochs"] + 1):
        ...
        for raw_source, target in train_loader:
            ...
            outputs = model.forward_train(source, pseudo_target, use_dpe=use_dpe)

            # Task loss — supervise both branches.
            # logits_tilde: LPE + DCM path (unavailable at inference but trains student backbone)
            # logits_hat: DPE + DCM path (inference branch)
            # Both contribute equally since they share the student encoder/decoder.
            if use_dpe:
                task = 0.5 * (
            ...
            # Prior constraint + distillation (both branches)
            # FIX: prior_mode_lpe_grad=True so LPE receives gradient from prior_loss.
            # This lets LPE learn to produce z_tilde that DPE can reliably predict.
            # Also warm up distill_weight together with prior_weight to avoid shock.
            if use_dpe:
        ...
        logging.info(
            "pdaf epoch=%03d loss=%.4f task=%.4f sc=%.4f kl=%.4f prior=%.4f distill=%.4f dice=%.4f soft_dice=%.4f iou=%.4f auc=%.4f",
            epoch,
            row["loss"],
            row["task_loss"],
            row["sc_loss"],
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train PDAF v2 — fixed normalize, prior loss gradient, DPE dual-mode.")
    parser.add_argument("--config", type=Path, default=Path("hyps/unet.yaml"))
...
def main() -> None:
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler(output_dir / "train_v2.log", encoding="utf-8"),
        ],
    )
